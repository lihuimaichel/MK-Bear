...
abstract class CApplication extends CModule
{
	...
	/**
	 * @desc 创建远程调取链接(iframe)
	 * @param string $route
	 * @param array $params
	 * @param string $ampersand
	 */
	public function createRemoteUrl($route,$server,$params=array(),$ampersand='&')
	{
	    $params['remote'] = 1;
	    $url = $this->getUrlManager()->createUrl($route,$params,$ampersand);
	    $host = SysConfig::getServerHost($server);
	    if(!$host){
	        throw new CException(Yii::t('system','Server Config Error'));
	    }
	    return $host.$url;
	}
