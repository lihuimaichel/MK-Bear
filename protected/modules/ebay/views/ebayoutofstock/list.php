...
$this->widget("UGridView", array(
	...
	'toolBar'		=>	array(
		array(
			...
			'url' 			=> 'ebay/ebayoutofstock/setoutofstock/',
			'htmlOptions' 	=> array(
					'class'     => 'add',
					'title'     => Yii::t('system', 'Sure you want to do this'),
					'target'    => 'selectedTodo',
					'rel'       => 'ebayoutofstock-grid',
					'postType'  => 'string',
		...
		array(
			...
			'url' 			=> 'ebay/ebayoutofstock/canceloutofstock/',
			'htmlOptions' 	=> array(
					'class' 	=> 'delete',
					'title'     => Yii::t('system', 'Sure you want to do this'),
					'target'    => 'selectedTodo',
	...
	'columns'		=>	array(
		...
		array(
			'name'  =>'is_outofstock',
			'value' => '$data->is_outofstock ? EbayOutofstock::getOutOfStockOption($data->is_outofstock) : ""',
			'type'  =>'raw',
			'htmlOptions' 		=> array(
					'style' => 'width:120px;',
			),
		...
		array(
			'name'  =>'ack',
			'value' => '$data->ack ? EbayOutofstock::getAckOption($data->ack) : ""',
			'type'  =>'raw',
			'htmlOptions' 		=> array(
					'style' => 'width:100px;',
			),
...
function selectAll(obj){
	...
	$("input[name='ebay_outofstock_list_c0[]']").not(":disabled").each(function(){
		this.checked = chcked;
	});
}
...
function checkSelect(obj) {
	if (!!$(obj).attr('checked')) {
		$(obj).parents('tr').find("input[name='ebay_outofstock_list_c0[]']").not(":disabled").each(function(){
			this.checked = true;
		});
	} else {
		$(obj).parents('tr').find("input[name='ebay_outofstock_list_c0[]']").not(":disabled").each(function(){
