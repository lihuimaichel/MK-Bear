	    ...
	    <div class="dot7" style="padding:5px;">
	       ...
	       <?php
            $form = $this->beginWidget('ActiveForm', array(
                ...
                'action' => Yii::app()->createUrl('ebay/ebayaccountsite/savedata'), 
                'htmlOptions' => array(        
                    'class' => 'pageForm',         
                )
            ));
            ?> 
            	<div class="formBar">
                    <ul> 
                        ...
                        <a href="<?php echo Yii::app()->createUrl('ebay/ebayaccountsite/index');?>" target="navTab" style="display:none;" class="display_list"><?php echo Yii::t('system', 'Save');?></a>
                    </ul>
                </div>
        		<table class="dataintable_inquire productAddInfo" width="100%" cellspacing="1" cellpadding="3" border="0">
				    <tbody>
                        ...
                        <tr>
				            <td width="15%" style="font-weight:bold;"><?php echo $account['short_name'];?></td>
				            <td>
				            	<?php foreach ($siteList as $key => $site):?>
				                <div style="float:left; display:inline; width:150px;">
                             		<?php echo CHtml::label($site, $account['short_name'] . '_' . $site, array('style'=>'float:right')); ?>
                             		<?php echo CHtml::checkBox("account[{$account['id']}][site_id][{$key}]", in_array($key, $account['select_site_id']), array('value'=>$key,'id'=>$account['short_name'] . '_' . $site)); ?>
                             		<?php echo CHtml::dropDownList("account[{$account['id']}][warehouse_id][{$key}]", isset($account['select_warehouse_id'][$key]) ? $account['select_warehouse_id'][$key] : 0, $wareHouseList, array('empty'=>array(''=>"请选择")));?>
                            	</div>
                            	<?php endforeach;?>
				            </td>
				        </tr>
