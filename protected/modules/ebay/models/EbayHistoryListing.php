...
class EbayHistoryListing extends EbayModel
{
	const TABLE_NAME = 'ueb_ebay_task_history_listing';
	 ...
	 * @return mixed
	 *
	 * 根据日期，平台更新历史记录中任务为系统记录标识
	 */
	public function syncSystemListing($date_time, $platform = 'EB')
	{
	...
	{
		$product_table = UebModel::model('EbayProductAdd')->tableName();
		$wait_table = $this->waitTable();
		$connection = Yii::app()->db_ebay;
	...
	public function fetchListingNum($seller_user_id, $date_time)
	{
		...
		$sql = "SELECT COUNT(id) AS total FROM {$table} WHERE 1 AND seller_user_id = {$seller_user_id} AND status = 3 AND date_time = '{$date_time}'";
	...
	{
		return EbayWaitListing::TABLE_NAME;
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'product_status' => Yii::t('ebay', 'Status'),
	...
	public function filterOptions()
	{
		...
		$group = $this->group();
        $account_list = EbayAccount::getIdNamePairs();
		...
		if ($group) {
            ...
            if ($job_id == ProductsGroupModel::GROUP_SALE) {
                //销售
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, Yii::app()->user->id);
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, Yii::app()->user->id, 'site');
                if (!empty($site_rows)) {
                ...
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, $users_rows);
                if (!empty($account_rows)) {
                    foreach ($account_rows as $ak => $av) {
                        $account_dropdown_list[$av['account_id']] = isset($account_list[$av['account_id']]) ? $account_list[$av['account_id']] : 'unkonw';
                    }
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, $users_rows, 'site');
                if (!empty($site_rows)) {
                    foreach ($site_rows as $sk => $sv) {
                        $site_dropdown_list[$sv['site']] = $sv['site'];
                    }
            ...
            if ($check_result) {
                ...
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, $users);
                if (!empty($account_rows)) {
                    foreach ($account_rows as $ak => $av) {
                        $account_dropdown_list[$av['account_id']] = isset($account_list[$av['account_id']]) ? $account_list[$av['account_id']] : 'unkonw';
                    }
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_EBAY, $users, 'site');
                if (!empty($site_rows)) {
                    foreach ($site_rows as $sk => $sv) {
                        $site_dropdown_list[$sv['site']] = $sv['site'];
                    }
