 ...
 * @author Gordon
 */
class EbayModel extends UebModel {
	const STATUS_WAIT = 1;
     ...
     * 记录文件拉listing错误日志
     */
    public function writeListingErrorLog($accountID = 0,$info = ''){
        $logPath = Yii::getPathOfAlias('webroot').'/log/ebay/';
        if( !is_dir($logPath) ){
            mkdir($logPath, 0777, true);
        }
        $log = date('Y-m-d H:i:s').' - '.$accountID." - ";//时间，接口名
        $log .= 'Message:'.$info."\n";//错误信息
        $fileName = 'Listing-'.date('Y-m-d').'-'.$accountID.'.txt';
