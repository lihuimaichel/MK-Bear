 ...
 * Time: 11:09
 */
class EbaySalesExtend extends EbayModel
{
	...
	{
		return 'ueb_ebay_task_sales_extend';
	}
		...
		$row = $this->getDbConnection()
			->createCommand(" 
				...
				SELECT
					id
				FROM
					".EbaySalesTarget::model()->tableName()."
				WHERE
					1
    ...
    public function clearData()
    {
        $sql = "    SELECT DISTINCT
                    ...
                    FROM
                        ".$this->tableName()." AS ex
                    INNER JOIN ".EbaySalesTarget::model()->tableName()." AS sa ON ex.seller_user_id = sa.seller_user_id
