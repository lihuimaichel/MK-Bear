...
class Product extends ProductsModel {
     ...
     * @see CActiveRecord::tableName()
     */
    public function tableNameAvgCostEffectiveUpdate() {
        return 'ueb_product_avg_cost_effective_update';
    }

    /**
     * @desc sku的产品毛重变化数据库表名
     * @see CActiveRecord::tableName()
     */
    public function tableNameWeightEffectiveUpdate() {
        return 'ueb_product_weight_effective_update';
    }
	 ...
	 * @param array $skuInfo
	 */
	public function getRealSkuListNew($sku, $qty=1, $skuInfo=null){
		if (empty($skuInfo)) {
			$skuInfo = $this->getProductInfoBySku($sku);
		}
	    if( !empty($skuInfo['combine_product']) && intval($skuInfo['combine_product']['quantity']) > 1){
	...
	*/
     ...
     * @return boolen
     */
    public function checkProfitRate($currency, $platformCode, $sku, $salePrice, $commissionRate = null, $shipWarehouseID = null, $siteID = null){
    	$result = true;
    	//通过平台缩写取出最小毛利率
	...
	public function getFullProductInfoBySku($sku)
    {
        $queryBuilder = $this->getDbConnection()->createCommand()
            ->select('p.id,p.sku,p.product_is_multi, p.product_category_id,c.category_cn_name,p.product_status,
            p.product_cost, p.avg_price,
            p.product_length, p.product_width, p.product_height,
            p.pack_product_length, p.pack_product_width, p.pack_product_height,
            d.title AS english_title,d.description as english_description,
            d.included as english_included,
            d2.title AS chinese_title,d2.description as chinese_description,
            d2.included as chinese_included,
            p.product_cost,p.gross_product_weight,
            p.product_weight,i.security_level,i.infringement')
            ->from(Product::model()->tableName() .' p')
            ->leftJoin(ProductInfringe::model()->tableName() .' i', "p.sku=i.sku")
            ->leftJoin(ProductCategory::model()->tableName().' AS c', "p.product_category_id=c.id")
            ->leftJoin( Productdesc::model()->tableName(). ' AS d', "p.id=d.product_id")
            ->leftJoin( Productdesc::model()->tableName(). ' AS d2', "p.id=d2.product_id")
           // ->where('p.sku = "' . $sku . '"')
            ->where("d.language_code = 'english'")
            ->andWhere("d2.language_code = 'Chinese'");
        ...
        if (is_array($sku)) {
            ...
            $queryBuilder->andWhere('p.sku =:sku', array(':sku'=> $sku));
