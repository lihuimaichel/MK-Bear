				            ...
				            <td>
					            <table id="skuAttributeTable" class="attributesTable">
						            <tr>
						            	...
						            	<th>
						            		<ul>
			            	 					<?php  foreach ($advertVariant as $sonK=>$sonSku){?>
	                	 				...
	                	 				<td>
                	 						...
                	 						<?php foreach ($advertVariant as $sonK=>$sonSku){?>
	                	 						...
	                	 						<?php if($value->hasvalues==1){?>
													<select name="advert[<?php echo $sonK; ?>][<?php echo $value->key;?>]" <?php if($value->mandatory==1){?>class="add_require"<?php }?>>
														<option value="">请选择</option>
														<?php foreach ($value->valueslist->value as $v) { ?>
														<option value="<?php echo $v?>" <?php if(array_key_exists(strval($value->key), $sonSku) && $v==$sonSku["$value->key"]){?>selected<?php }?>><?php echo $v?></option>
														<?php }?>
													</select>
												<?php }else{ ?>
													<input type="text" name="advert[<?php echo $sonSku['sellerReference']; ?>][<?php echo $value->key;?>]" class="<?php if($value->mandatory==1){?>add_require<?php }?> textInput"
													<?php if(array_key_exists(strval($value->key), $sonSku)){?>value="<?php echo $sonSku["$value->key"]; ?>"<?php }?>>
												<?php }?>
			                    	 				...
			                    	 				<td>
		                    	 						<?php if($value->hasvalues==1){?>
		                    	 							<select name="product[<?php echo $value->key;?>]" <?php if($value->mandatory==1){?>class="add_require"<?php }?>>
																<option value="">请选择</option>
																<?php foreach ($value->valueslist->value as $v) { ?>
																<option value="<?php echo $v?>" <?php if(array_key_exists(strval($value->key), $pmExtend['product_desc']) && $v==$pmExtend['product_desc']["$value->key"]){?>selected<?php }?>><?php echo $v?></option>
																<?php }?>
															</select>
		                    	 						<?php }else{ ?>
															<input type="text" name="product[<?php echo $value->key;?>]" class="<?php if($value->mandatory==1){?>add_require<?php }?> textInput" <?php if($value->key=='title' || $value->key=='titre'){?> style="width:500px" <?php }?>
															<?php if(array_key_exists(strval($value->key), $pmExtend['product_desc'])){?>value="<?php echo $pmExtend['product_desc']["$value->key"]; ?>" <?php }?>>
		                    	 						<?php }?>
