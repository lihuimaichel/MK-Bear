 ...
 *
 */
class PriceministerstocklogController extends UebController{
	public function actionList(){
		$this->render("list", array(
			"model"	=>	new PriceministerStockLog()
		));
	}

	/*
	 * 	定时脚本,获取修改listing结果
	 * 	访问库存日志表数据拿到结果，成功更新数据
	 * 	/priceminister/priceministerstocklog/updateStockLog/account_id/1/file_id/7209737
	 */
	public function actionUpdateStockLog(){
		...
		if($accountID){
			try{
				...
				do{
					$command = $pmStockLogModel->getDbConnection()->createCommand()
						->from($pmStockLogModel->tableName() . " as t")
					...
					if($productList){
						...
						foreach($productList as $info){
							...
							if($error_msg==''){//成功
								$updateData = array(
									'status'=>PriceministerStockLog::STATUS_SUCCESS,
									'msg'=>'ok',
									'update_time'=>date("Y-m-d H:i:s"),
								);
							}else{
								$updateData = array(
									'status'=>PriceministerStockLog::STATUS_FAILURE,
									'msg'=>$error_msg,
									'update_time'=>date("Y-m-d H:i:s"),
								);
							}
							//更新日志
							$pmStockLogModel->getDbConnection()->createCommand()
								->update($pmStockLogModel->tableName(), $updateData,"id = " . $info['id'] );
						}
					}else{
						$isContinue = false;
					}
