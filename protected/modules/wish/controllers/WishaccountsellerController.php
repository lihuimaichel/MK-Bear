 ...
 *
 */
class WishaccountsellerController extends UebController {
	...
	public function init() {
		$this->_model = new WishAccountSeller();
	}
	...
	public function actionList() {
		$aliAccountModel = new WishAccount();
	...
	public function actionAdd(){
		if($_POST){
			$sellerList     = Yii::app()->request->getParam('WishAccountSeller');
			...
			$jsonData = array(
				'message' => '保存成功',
				'forward' =>'/wish/wishaccountseller/list',
				'navTabId'=> 'page' . WishAccountSeller::getIndexNavTabId(),
	...
	public function actionAjaxdata(){
		...
		foreach ($accountList as $key => $value) {
			...
			$data .= '<span><input name="WishAccountSeller[account_id][]" type="checkbox" value="'.$key.'" '.$select.' />'.$value.'</span>';
		}
