 ...
 * @since 2015-06-22
 */
class WishAccount extends WishModel{
    ...
    /** @var tinyint 深圳wish部*/
    const DEPARTMENT_SHENZHEN = 15;

    /** @var tinyint 长沙wish部*/
    const DEPARTMENT_CHANGSHA = 37;
    ...
    public static function getAbleAccountList(){
        return WishAccount::model()->dbConnection->createCommand()
                       ->select('*')
                       ->from(self::tableName())
    ...
    public static function getAccountInfoById($id){
        return WishAccount::model()->dbConnection->createCommand()->select('*')->from(self::tableName())->where('id = '.$id)->queryRow();
    }
    ...
    public static function getByAccountName($accountName) {
    	return WishAccount::model()->getDbConnection()->createCommand()
		    	->select('*')
		    	->from(self::tableName())
    ...
    public static function getAbleAccountListByFilterId($ids){
    	if(is_array($ids) && $ids)
    		$ids = implode(",", $ids);
    	$command = WishAccount::model()->dbConnection->createCommand()
					    	->select('*')
					    	->from(self::tableName())
					    	->order("account_name asc")
    ...
    public static function getAccountInfoByIds($id){
    	...
    	$sql = WishAccount::model()->dbConnection->createCommand()->select('*')->from(self::model()->tableName())->where('id IN ('.implode(',', $id).')');
    	if( $flag ){
    		return $sql->queryRow();
    	}else{
    		return $sql->queryAll();
	...
	public static function getIndexNavTabId() {
		return Menu::model()->getIdByUrl('/Wish/wishaccount/list');
	}
	/**
	...
	public function getDepartment($department_id = null){
		$list = array(self::DEPARTMENT_SHENZHEN=>'深圳wish部', self::DEPARTMENT_CHANGSHA=>'长沙wish部');
    ...
    public function findAccountListByDepartmentId($departmentId)
    {
        $queryBuilder = $this->getDbConnection()->createCommand()->from($this->tableName())
            ->select('id, account_name as short_name, account as account_name, department_id')
