 ...
 * @since 2016-03-28
 */
class WishProductSellerRelation extends WishModel{
	public $account_name;
    ...
    public function tableName() {
        return 'ueb_wish_product_seller_relation';
    }
    ...
    public function batchChangeSellerToOtherSeller($oldAccountId, $oldSellerId, $newSellerId){
        $dbtransaction = WishProductSellerRelationLog::model()->getDbConnection()->beginTransaction();
        try {
    ...
    public function batchSetAccountListingToSeller($accountID, $sellerID){
    	...
    	do{
	    	$productList = $this->getDbConnection()->createCommand()
	    							->from(WishVariants::model()->tableName()." as p")
	    							->leftJoin($this->tableName() . " as s", "s.account_id=p.account_id and s.online_sku=p.online_sku and s.item_id=p.product_id")
	    							->select("p.product_id as item_id,p.sku,p.online_sku,p.account_id")
    ...
    public function batchSetSkuListingToSeller($ids, $sellerID){
    	...
    	$productList = $this->getDbConnection()->createCommand()
				    		->from(WishVariants::model()->tableName()." as p")
				    		->leftJoin($this->tableName() . " as s", "s.account_id=p.account_id and s.online_sku=p.online_sku and s.item_id=p.product_id")
				    		->select("p.product_id as item_id,p.sku,p.online_sku,p.account_id, s.seller_id")
    ...
    public static function getIndexNavTabId() {
        return Menu::model()->getIdByUrl('/wish/wishproductsellerrelation/list');
    }
    ...
    public static function getUnbindsellerNavTabId() {
        return Menu::model()->getIdByUrl('/wish/wishproductsellerrelation/unbindseller');
    }
    ...
    public function getUnBindSellerListByCondition($conditions, $params){
        return $this->getDbConnection()->createCommand()
                    ->select('p.product_id as item_id, p.sku, p.online_sku as sku_online, p.account_id')
                    ->from(WishVariants::model()->tableName(). ' as p')
                    ->leftJoin($this->tableName(). ' as s', 's.account_id=p.account_id and s.online_sku=p.online_sku and s.item_id=p.product_id')
                    ->where($conditions, $params)
    ...
    public function findRelationByAccountAndItem($accountIds, $itemIds)
    {
        $queryBuilder = WishProductSellerRelation::model()->getDbConnection()->createCommand()
            ->from( WishProductSellerRelation::model()->tableName())
            ->select('account_id, item_id, sku, seller_id, online_sku')
            ->where('account_id  IN ('.MHelper::simplode(array_unique($accountIds)).')')
            ->andWhere('item_id IN ('.MHelper::simplode(array_unique($itemIds)).')')
            ->group('seller_id');
