 ...
 * @since 2015-06-22
 */
class WishSpecialOrderBuyerIdStatistic extends WishModel{
	/** @var tinyint 未付款*/
	...
	public function getAccessUser(){
		$id = Yii::app()->user->id;
		if(in_array($id, array(1, 4, 17, 1373, 1017))){
			return true;
		}
		exit('待处理');
	}
	...
	public function search($buyerId){
		$sort = new CSort();
		$sort->attributes = array(
				'defaultOrder'=>'t.order_id',
		);
	
		$dataProvider = parent::search($this, $sort, '', $this->_setDbCriteria($buyerId));
	...
	private function _setDbCriteria($buyerId){
		/**
		SELECT buyer_id, COUNT(o.order_id) as order_count, COUNT(d.id) as sku_count, SUM(o.total_price) as price_count, DATE_FORMAT(o.ori_pay_time, '%Y-%m-%d') as paydate
		FROM `ueb_wish_order` o
		JOIN `ueb_wish_order_detail` d on d.order_id=o.order_id
		WHERE o.buyer_id='574d2a7a34220519387dcec6'
		GROUP BY paydate;
		 */
		$cdbCriteria = new CDbCriteria();
		$cdbCriteria->select = "t.buyer_id, COUNT(DISTINCT t.order_id) as order_count, COUNT(d.sku) as sku_count, SUM(d.total_price+d.ship_price) as price_count, DATE_FORMAT(t.ori_pay_time, '%Y-%m-%d') as paydate";
		$cdbCriteria->group = "paydate";
		$cdbCriteria->join = "join `ueb_wish_order_detail` d on d.order_id=t.order_id";
		//只统计当前一月内的
		$beforeData = date("Y-m-d 00:00:00", time()-30*24*3600);
		$cdbCriteria->addCondition("t.ori_pay_time >= '{$beforeData}'");
		$cdbCriteria->addCondition("t.buyer_id = '{$buyerId}'");
