...
$this->widget('UGridView', array(
	...
	'toolBar' => array(
		array(
			...
			'url' => Yii::app()->createUrl('wish/wishproductstatistic/batchpublish/', array( 'account_id' => $accountID)),
			'htmlOptions' => array(
				'class' => 'add',
				'target' => 'selectedTodo',
				'rel' => 'wish_product_statistic',
				'postType' => 'string',
...
<script>
	...
	(function(form) {
		...
		var checkSku = function(check) {
			...
			if(skuVal != '') {
				...
				if(isNaN(skuVal)) {
					...
					alertMsg.error("<?php echo Yii::t('wish_product_statistic', 'Invalid sku');?>");
					$('#sku').focus();
					return false;
				}
			}
		...
		var errorCostInput = function(formObj) {
			alertMsg.error("<?php echo Yii::t('wish_product_statistic', 'Invalid product cost input');?>");
