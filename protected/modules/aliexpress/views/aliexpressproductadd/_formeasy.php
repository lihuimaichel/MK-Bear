...
if (isset($variationAttributes) && !empty($variationAttributes)) {
	foreach ($variationAttributes as $skuAttribute) {
		...
		echo '_variationSkus[' . $key . '].customName = \'' . str_replace("'","\'",$skuAttribute['custom_name']) . '\'' . "\n";
	}
}
?>
//设置选中分类
...
<?php $getVariationProductPriceByChangeOverseasWarehouseJS = ''; ?>
//有海外仓
//By AjunLongLive!
function getVariationProductPriceByChangeOverseasWarehouse() {
	$("input[name^='variation_price']").each(function(index,dom){
		getVariationProductPrice(index);
	});	
}

<?php $getProductPriceByChangeOverseasWarehouseJS = ''; ?>
//获取产品价格信息,当是修改海外仓选项
function getProductPriceByChangeOverseasWarehouse(OverseasWarehouseID) {
	if (_publishType != 1)
		 return false;
	var categoryID = $('input[name=category_id]').val();
	var sku = $('input[name=publish_sku]').val();
	var accounts = $('input[name=account_id\\[\\]]').serialize();
	var accountID = $('.tabsHeaderContent').find("li[class^='selected']").find("input[name^='account_id']").val(); console.log(accountID);
	var data = 'category_id=' + categoryID + '&publish_sku=' + sku + '&selected_account_id=' + accountID + '&overseas_warehous_id=' + OverseasWarehouseID + '&' + accounts;
	//return;
	$.ajax({
		type:'post',
		url:'aliexpress/aliexpressproductadd/getproductpricebychangeoverseaswarehouse',
		data:data,
		success:function(result){
			//加载卖价
...
function getVariationProductPrice(row) {
	...
	var prices = $.trim($('input[name=variation_price\\[' + row + '\\]]').val());
	console.log(sku);
	if (sku == '')
		return false;
	...
	*/
	...
	var account_id = 0;
	var data = 'category_id=' + categoryID + '&publish_sku=' + sku + '&' + 'account_id[]=' + account_id + '&price=' + prices;

	var overseas_warehous_id = $('#overseas_warehouse_id').val();
	console.log(overseas_warehous_id);
	var postUrl = 'aliexpress/aliexpressproductadd/getproductprice';
	if (overseas_warehous_id != undefined && overseas_warehous_id != ''){
		postUrl = 'aliexpress/aliexpressproductadd/getproductpricebychangeoverseaswarehouse';
		data = 'category_id=' + categoryID + '&publish_sku=' + sku + '&' + 'account_id[]=' + account_id + '&price=' + prices + '&overseas_warehous_id=' + overseas_warehous_id + '&selected_account_id=' + account_id;
	}
...
function generateVariationProduct() {
	...
	if (_publishType == 2 || variationPublishFlag) {
		...
		for (var i in rows) {
			...
			html += '<td><input type="text" class="sku_variation_price" onchange="getVariationProductPrice(' + i + ')" name="variation_price[' + i + ']" value="' + price + '" size="8" />';
				    ...
				    <tbody>
				    	...
				    	<tr>
				            ...
				            <td>
				                ...
				                <input type="hidden" name="id" value="<?php echo $addInfo['id'];?>" />
				                <?php } else { ?>
				            ...
				            <td>
				                <div class="page unitBox ztimgs">
                                    ...
                                    <div class="sortDrag sortDragShow" style="width:99%;margin:5px;min-height:100px">
                                    	...
                                    	<?php if (isset($selectedImages['zt'])) if ($action == 'update' && in_array($k, $selectedImages['zt'])):?>
                                        <?php $selected++;?>
                                        <?php endif;?>
                        ...
                        <tr id="skuAttrRow"<?php echo ($action != 'update') ? ' style="display:none"' : '';?>>
				            ...
				            <td>
				                ...
				                <div id="skuOverseasWarehouse">
				                <table id="skuOverseasWarehouseTable" class="attributesTable">
									<tbody>
										<!-- 多属性海外仓 -->
										<?php if ($publishParams['publish_type']['id'] == 2 && isset($overseasWarehouseList) && !empty($overseasWarehouseList)){ ?>
										<tr id="attr_overseas_warehouse">
											<td class="leftColumn">
												<label>海外仓：</label>
											</td>
											<td class="rightColumn">
												    <select name="overseas_warehouse_id" 
                            	            		        id="overseas_warehouse_id" style="float:left;" 
                            	            		        onChange="getVariationProductPriceByChangeOverseasWarehouse()">
                            	            			<option value=""><?php echo Yii::t('system', 'Please Select')?></option>
                            	            			<?php foreach ($overseasWarehouseList as $warehouseID => $warehouseName) { ?>
                            	            			    <option value="<?php echo $warehouseID; ?>" 
                            	            			    <?php echo (isset($addInfo['overseas_warehouse_id']) && $addInfo['overseas_warehouse_id'] == $warehouseID) ? 'selected="selected"' : ''; ?>
                            	            			    ><?php echo $warehouseName; ?></option>
                            	            			<?php } ?>
                            	            		</select>
											</td>
										</tr>
                            	 	...
                            	 	<div class="tabsContent"> 
                            	 	    ...
                            	 	    <table class="baseinfoTable" width="98%">
                             			    ...
                             			    <tr>
                             			    	...
                             			    	<td>
                             			    		<input type="text" group="subject" onchange="autoFill(this)" name="subject[<?php echo $accountID?>]" value="<?php echo $account['product_title'];?>" onblur = "checkStrLength(this,128)" maxlength="128" size="125"/>
                             			    		&nbsp;&nbsp;<span class="warn" style="color:red;line-height:22px;"></span>
                             			    	</td>
                            	            </tr>
                            	            <!-- <tr>
                             			        <td>
                             			    		<span><?php echo Yii::t('aliexpress_product', 'Product Keywords');?>：</span>
                             			    	</td>
                             			    	<td>
	                             			    	<input group="keywords" onchange="autoFill(this)" type="text" name="keywords[<?php echo $accountID?>]" style="float:none;" value="<?php echo $account['product_keywords'];?>" size="25"/>
	                             			    	&nbsp;&nbsp;<input group="more_keywords1" onchange="autoFill(this)" type="text" class="textInput" style="float:none;" name="more_keywords1[<?php echo $accountID?>]" value="<?php echo $account['product_keywords1'];?>" size="25" />
	                             			    	&nbsp;&nbsp;<input group="more_keywords2" onchange="autoFill(this)" type="text" class="textInput" style="float:none;" name="more_keywords2[<?php echo $accountID?>]" value="<?php echo $account['product_keywords2'];?>" size="25" />
                             			    	</td> -->
                            	            </tr>
                            	            ...
                            	            <tr>
                            	            	...
                            	            	<td>
                            	            		<select name="service_template_id[<?php echo $accountID?>]" id="service_template_id" style="float:left;">
                            	            			<option value=""><?php echo Yii::t('system', 'Please Select')?></option>
                            	            			<?php echo $account['service_list'];?>
                            	            		</select><div id="updateService" class="categoryBox"><a href="javascript:void(0)" class="btn">更新服务模板</a></div>
                            	            	</td>
                            	            </tr>
                            	            <?php if ($publishParams['publish_type']['id'] == 1 && isset($account['overseas_warehouse_list']) && !empty($account['overseas_warehouse_list'])) { ?>
                            	            <tr>
                            	            	<td><span>海外仓库</span></td>
                            	            	<td>
                            	            		<select name="overseas_warehouse_id" 
                            	            		        id="overseas_warehouse_id" style="float:left;" 
                            	            		        onChange="getProductPriceByChangeOverseasWarehouse($(this).val())">
                            	            			<option value=""><?php echo Yii::t('system', 'Please Select')?></option>
                            	            			<?php foreach ($account['overseas_warehouse_list'] as $warehouseID => $warehouseName) { ?>
                            	            			    <option value="<?php echo $warehouseID; ?>" 
                            	            			    <?php echo (isset($addInfo['overseas_warehouse_id']) && $addInfo['overseas_warehouse_id'] == $warehouseID) ? 'selected="selected"' : ''; ?>
                            	            			    ><?php echo $warehouseName; ?></option>
                            	            			<?php } ?>
				            		...
				            		<table class="baseinfoTable">
				            			<tbody>
				            				...
				            				<tr>
				            					...
				            					<td class="rightColumn">
				            						<input size="6" type="text" name="lot_num" value="<?php echo isset($addInfo['lot_num']) ? $addInfo['lot_num'] : '';?>" />&nbsp;&nbsp;
				            				...
				            				<tr>
				            					...
				            					<td class="rightColumn"><input size="6" type="text" name="gross_weight" value="<?php echo $action == 'update' ? $addInfo['gross_weight'] : $skuInfo['gross_weight'];?>" />&nbsp;&nbsp;<?php echo Yii::t('aliexpress', 'Unit Kilogram');?></td>
				            				</tr>
				            				<tr>
				            					<td class="leftColumn"><label><?php echo Yii::t('aliexpress', 'Product Package Size');?></label></td>
				            					<td class="rightColumn">
				            						<ul class="productSize">
				            							<li><input type="text" size="6" name="package_length" value="<?php echo $action == 'update' ? $addInfo['package_length'] : $skuInfo['package_length'];?>" /></li>
				            							<li><input type="text" size="6" name="package_width" value="<?php echo $action == 'update' ? $addInfo['package_width'] : $skuInfo['package_width'];?>" /></li>
				            							<li><input type="text" size="6" name="package_height" value="<?php echo $action == 'update' ? $addInfo['package_height'] : $skuInfo['package_height'];?>" />&nbsp;&nbsp;<?php echo Yii::t('aliexpress', 'Unit Centimeter');?></li>
				            						</ul>
				            					</td>
				            				</tr>				            							            				
				            			</tbody>
...
$(function(){
	...
	//$('#ajunBrandSelectCategory').css('width',$('#ajunBrandSelect').width());
	setTimeout(function(){
