 ...
 * @since 2015-06-25
 */ 
class AliexpressLog extends AliexpressModel{
    ...
    //标记发货
    const EVENT_ADVANCE_SHIPPED = 'advance_shipped';

    //上传跟踪号
    const EVENT_UPLOAD_TRACK = 'upload_track';

    //下载订单
    const EVENT_ORDER_DOWNLOAD = 'order_download';

    //补拉近2天订单
    const EVENT_ORDER_WAIT     = 'get_waitorders';    

    //补拉订单
    const EVENT_CHECK_DOWNLOADORDER = 'check_downloadorder';

    const EVENT_SYNCORDER     = 'sync_order';

    //获取建议分类
    const EVENT_GET_CATEGORIES_SUGGEST = 'get_categories_suggest';

    /** @var 清除三个月前日志 */
    const EVENT_CLEAR_LOG = 'clear_log';

    /** @var 产品改价日志 */
    const EVENT_DISCOUNT_SET_PRICE_LOG = 'discount_set_price';
    ...
    public function saveEventStatus($eventName, $logID, $status,$message = ''){
        ...
        if( $eventName == AliexpressShipment::EVENT_ADVANCE_SHIPPED || $eventName == AliexpressShipment::EVENT_UPLOAD_TRACK ){
        	$updateData =  array(
        			'status'        => $status,
        			'end_time'      => date('Y-m-d H:i:s'),
        			'message'		=> $message,
    ...
    public function getAccountList(){
            if(self::$accountPairs == null)
                    self::$accountPairs = self::model('AliexpressAccount')->getIdNamePairs();
    ...
    public function getEventOptions($event = null){
            ...
            $eventOptions = array(
                ...
                self::EVENT_GET_CATEGORIES_SUGGEST  => '更新产品类目',
                self::EVENT_GET_PRODUCT             => '拉取listing',
                self::EVENT_ADVANCE_SHIPPED         => '标记已发货',
                self::EVENT_UPLOAD_TRACK            => '上传跟踪号',
                self::EVENT_ORDER_DOWNLOAD          => '下载订单',
                self::EVENT_ORDER_WAIT              => '补拉订单-1',
                self::EVENT_CHECK_DOWNLOADORDER     => '补拉订单-2',
