 ...
 * @since 2015-06-25
 */
class AliexpressAccount extends AliexpressModel{
    ...
    {
        $account_rows = self::getAbleAccountList();
        $account_array = array();
        if (!empty($account_rows)) {
            foreach ($account_rows as $ak => $av) {
                $account_array[$av['account']] = $av['id'];
            }
        }
     ...
     * @param int $id
     */
    public static function getAccountOvseasWarehouseListById($id){
        $hasSetOvseasWarehouse = self::model()->dbConnection->createCommand()
                    ->select("id")
                    ->from(self::model()->tableName())
                    ->where(" id={$id} and is_overseas_warehouse=1 ")
                    ->queryRow();
        if ($hasSetOvseasWarehouse){
            return AliexpressOverseasWarehouse::getWarehouseList();
        } else {
            return false;
        }
    }
     ...
     * @param type $status
     */
    public static function getWarehouseLable($status) {
    	if ($status == self::STATUS_OPEN) {
    		echo '<font color="green" >海外仓</font>';
    	} else {
