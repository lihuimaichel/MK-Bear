...
class AliexpressOverseasWarehouse extends AliexpressModel { 
    ...
    public function tableName() {
        return 'ueb_aliexpress_overseas_warehouse_map';
    }
    ...
    public static function getWarehouseAllList(){
       /*  return array(
        ...
        $wishWarehouseConfigModel = new AliexpressWarehouseConfig();
    ...
    public function saveDataByExcel($excelFile){
        ...
        for($i=2;$i<= count($excelData);$i++){
            ...
            if (!empty($accountName)) {
                $accountInfo = AliexpressAccount::model()->getInfoByShortName(addslashes($accountName));
                if($accountInfo) $account_id = $accountInfo['id'];
            }
            
            //echo '$seller'.$seller;  
            //echo '$seller_id'.$seller_id;  echo '$account_id'.$account_id;
            
            if (empty($seller_id) || empty($account_id) || $overseasWarehouseID <= 0){
