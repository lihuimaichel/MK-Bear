 ...
 *
 */
class AliexpressProductDownloadNotAutomatic extends AliexpressProduct {
	...
	public function getAliProducts() {
		...
		while ( $request->getPage() <=  $request->getTotalPage() ) {
			...
			//print_r($response);
			/*
			$request->setRequest()->setUrl();
			$postUrl = $request->getUrl();
			$postBody = $request->getRequestBody();
			echo $postUrl . "\n";
			print_r($postBody);
			echo "\n";
			...
			*/
			...
			if (!$request->getIfSuccess()) {
				...
				return '请求接口失败!====='.$request->getErrorMsg();
				break;
			}
			...
			if (empty($productList)) {
			    //return "api接口aeopAEProductDisplayDTOList返回错误：产品ID->". $productId . '。' ;
				//break;
			}
			/* var_dump($totalPage);
			...
			foreach ($productList as $product) {
				...
				try {
					...
					if (!$aliProductId){
						...
						return "保存产品信息错误：id->" . $aliProductId . '。' . $aliexpressProduct->getErrorMessage();
						//continue;
                    }
					...
					if (!$flag){
						...
						return "保存产品Extend错误：id->" . $aliProductId . '。' . $aliProductExtend->getErrorMessage();
						//continue;
                    }
					...
					if (!$flag){
						...
						return "保存产品Variation错误：id->" . $aliProductId . '。' . $aliProductVariation->getErrorMessage();
						//continue;
                    }
					...
					return $e->getMessage().'--- 产品ID:'.$productID;
					//$errorList['productID']["$productID"] = $errMsgs;
