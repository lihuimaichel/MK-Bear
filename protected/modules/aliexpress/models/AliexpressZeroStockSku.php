 ...
 *
 */
class AliexpressZeroStockSku extends AliexpressModel {
	/** @var 把库存置为0 */
	const EVENT_ZERO_STOCK = 'zero_stock';
	/** @var 恢复库存 */
	const EVENT_RESTORE_STOCK = 'restore_stock';

	public $operation_user_id;	
	...
	public function tableName() {
		return 'ueb_aliexpress_zero_stock_sku';
	}
	 ...
	 */
	public function getZeroSkuListByCondition($conditions, $params, $limit=''){
	...
	/**
	 * @desc  通过seller_sku和product_id检测当天是否已经运行了
	 * @param unknown $sellerSku
	 * @param unknown $productID
	 * @return boolean
	 */
	public function checkHadRunningBySellerSkuAndproductID($sellerSku, $productID){
		$command = $this->getDbConnection()
			->createCommand()
			->from($this->tableName())
			->select('id')
			->where("seller_sku=:seller_sku AND product_id=:product_id", array(':seller_sku'=>$sellerSku, ':product_id'=>$productID))
			->andWhere("status=".self::STATUS_SUCCESS);
		$res = $command->queryRow();
		if($res)
			return true;
