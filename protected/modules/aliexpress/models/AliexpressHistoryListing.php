...
class AliexpressHistoryListing extends AliexpressModel
{
	const TABLE_NAME = 'ueb_aliexpress_task_history_listing';
	...
	public function saveData($values)
	{
		...
		$connection = Yii::app()->db_aliexpress;
	...
	{
		$wait_table = AliexpressWaitListing::TABLE_NAME;
	...
	{
		$product_table = UebModel::model('AliexpressProductAdd')->tableName();
	...
	{
		return AliexpressWaitListing::TABLE_NAME;
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'product_status' => Yii::t('aliexpress', 'Status'),
	...
	public function filterOptions()
	{
		...
		$group = $this->group();
        $account_list = AliexpressAccount::getIdNamePairs();
		...
		if ($group) {
            ...
            if ($job_id == ProductsGroupModel::GROUP_SALE) {
                //销售
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, Yii::app()->user->id);
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, Yii::app()->user->id, 'site');
                if (!empty($site_rows)) {
                ...
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, $users_rows);
                if (!empty($account_rows)) {
                    foreach ($account_rows as $ak => $av) {
                        $account_dropdown_list[$av['account_id']] = isset($account_list[$av['account_id']]) ? $account_list[$av['account_id']] : 'unkonw';
                    }
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, $users_rows, 'site');
                if (!empty($site_rows)) {
                    foreach ($site_rows as $sk => $sv) {
                        $site_dropdown_list[$sv['site']] = $sv['site'];
                    }
            ...
            if ($check_result) {
                ...
                $account_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, $users);
                if (!empty($account_rows)) {
                    foreach ($account_rows as $ak => $av) {
                        $account_dropdown_list[$av['account_id']] = isset($account_list[$av['account_id']]) ? $account_list[$av['account_id']] : 'unkonw';
                    }
                ...
                $site_rows = SellerUserToAccountSite::model()->getDistinctData(Platform::CODE_ALIEXPRESS, $users, 'site');
                if (!empty($site_rows)) {
                    foreach ($site_rows as $sk => $sv) {
                        $site_dropdown_list[$sv['site']] = $sv['site'];
                    }
