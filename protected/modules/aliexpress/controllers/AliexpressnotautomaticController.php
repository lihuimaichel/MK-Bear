<?php
/**
 * @desc Aliexpress手动拉取listing
 * @author AjunLongLive
 * @since 2017-02-22
 */
class AliexpressnotautomaticController extends UebController{
    ...
    public function actionDoAction(){
        $this->render("doActionIndex");
    }
    
    /**
     * @desc 手动拉取listing Ajax 返回
     */
    public function actionDoActionAjax(){
        ...
        $productIdList      = trim(Yii::app()->request->getParam('product_id_list',0));
        if (empty($accountId)){
            $return['status'] = 'failure';
            $return['msg'] = '账户不能为空！';
        }
        if ($productIdList == 0 || empty($productIdList)) {
            $returnApi = $this->getProductAndUpdateToSql($accountId,0);
            if ($returnApi['status'] == 'failure'){
                 $return['status'] = 'failure';
                 $return['msg'] = $returnApi['msg'];
            }
        } else {
            $productIdArr = explode(',', $productIdList);
            //print_r($productIdArr);
            foreach ($productIdArr as $productId) {
                if (trim($productId) != '') $returnApi = $this->getProductAndUpdateToSql($accountId,$productId);
                if ($returnApi['status'] == 'failure'){
                    $return['status'] = 'failure';
                    $return['msg'] = $returnApi['msg'];
                    break;
    ...
    /**
     * @desc 手动拉取listing  内部函数
     */
    public function getProductAndUpdateToSql($accountIdOut,$productIdOut){
        ...
        //start handle params
        if ( $accountId && empty($validateMsg)) {
            if (!empty($status)) {
            ...
            foreach ($statusTypeList as $statusType) {
                ...
                */
                //拉取产品
                $model = new AliexpressProductDownloadNotAutomatic();
