 ...
 *
 */
class AliexpressaccountsellerController extends UebController {
	...
	public function init() {
		$this->_model = new AliexpressAccountSeller();
	}
	...
	public function actionList() {
		...
		$sellerList      = User::model()->getUserNameByDeptID(array(4, 25));
	...
	public function actionAdd(){
		if($_POST){
			$sellerList     = Yii::app()->request->getParam('AliexpressAccountSeller');
			...
			$jsonData = array(
				'message' => '保存成功',
				'forward' =>'/aliexpress/aliexpressaccountseller/list',
				'navTabId'=> 'page' . AliexpressAccountSeller::getIndexNavTabId(),
	...
	public function actionAjaxdata(){
		...
		foreach ($accountList as $key => $value) {
			...
			$data .= '<span><input name="AliexpressAccountSeller[account_id][]" type="checkbox" value="'.$key.'" '.$select.' />'.$value.'</span>';
		}
