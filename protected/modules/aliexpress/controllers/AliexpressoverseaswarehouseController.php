<?php
/**
 * @desc   Aliexpress海外仓（产品ID）影射表
 * @author AjunLongLive!
 * @since  20170329
 */
class AliexpressoverseaswarehouseController extends UebController{
	/**
	 * @desc 访问过滤配置
	...
	public function actionList(){
		...
		$this->render('list',array('model'=>new AliexpressOverseasWarehouse(),'request'=>$request));
	}

	public function actionTouploadfile(){
		$this->render('uploadfile',array('model'=>new AliexpressOverseasWarehouse()));
	}	
	...
	public function actionDownloadTP(){
            ...
            $excelData->export_excel(array('系统SKU', '产品ID', '海外仓ID', '销售人员', '账号名称'),array(),'AliexpressOverseasWarehouse'.date('YmdHi').'.xls',$limit=10000,$output=1,$column_width);
            Yii::app()->end();
	}
	...
	public function actionUploadfile(){
		...
		if( !empty($_FILES) ){
			$models = new AliexpressOverseasWarehouse();
			$dir="uploads/";
			...
			$fileName = 'AliexpressOverseasWarehouse'.date('YmdHi');			
			...
			if($error == 'on_excel_data'){
				...
				$jsonData = array(
						...
						'forward' => '/aliexpress/aliexpressoverseaswarehouse/list',
						'navTabId' => 'page'.Menu::model()->getIdByUrl('/aliexpress/aliexpressoverseaswarehouse/list'),
	...
	public function actionDelete(){
		if (Yii::app()->request->isAjaxRequest && isset($_REQUEST['ids'])) {
			...
			if (!empty($ids) && AliexpressOverseasWarehouse::model()->deleteAll("id in (" . implode(',', $ids) . ")")) {
	...
	public function actionOutFile(){
        ...
        $AliexpressOverseasWarehouseModel = new AliexpressOverseasWarehouse();
        $ret = $AliexpressOverseasWarehouseModel->getListByCondition('*',$conditions,$data);
		...
		if ($ret){
			foreach ($ret as $val){
				...
				$accountName = '';
				if ($val['account_id'] > 0){
					$accountName = AliexpressAccount::model()->getAccountNameById($val['account_id']);
				}
		...
		$exportName = 'AliexpressOverseasWarehouse'.date('Ymd').'.csv';
	...
	public function actionUpdate(){
		...
		try{
			...
			$info = AliexpressOverseasWarehouse::model()->findByPk($id);
			if(empty($info)){
				throw new Exception(Yii::t('wish_product_statistic', 'Record does not exist'));
			}
			$this->render("update", array("model"=>$info, 'sellerList'=>User::model()->getAliexpressUserList(), 'accountList'=>AliexpressAccount::model()->getIdNamePairs(), 'warehouseList'=>AliexpressOverseasWarehouse::model()->getWarehouseList()));
		}catch (Exception $e){
	...
	public function actionSavedata(){
		try{
			$id                  = Yii::app()->request->getParam("id");
			$formData            = Yii::app()->request->getParam("AliexpressOverseasWarehouse");
			$sku                 = $formData['sku'];
			...
			$ret = AliexpressOverseasWarehouse::model()->updateListByCondition("id = {$id}", $updateData);
			if(!$ret){
