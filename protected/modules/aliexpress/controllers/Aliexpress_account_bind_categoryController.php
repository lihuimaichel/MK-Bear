 ...
 * @since 2015-09-11
 */
class Aliexpress_account_bind_categoryController extends UebController{
    ...
    /**
     * @desc Ajax判断分类对给定用户集合的绑定情况
     * /aliexpress/aliexpress_account_bind_category/check_category_bind_status
     */
    public function actionCheck_category_bind_status(){
        $nowTime = date("Y-m-d H:i:s",time());        
        $checkCategoryID = Yii::App()->request->getParam('categoryID');
        $checkUsers = Yii::App()->request->getParam('usersJson');
        $return = AliexpressAccountBindCategory::model()->checkCategoryBindStatusfromUsers($checkCategoryID,json_decode($checkUsers));
        echo json_encode($return);
    }    
    
    /**
     * @desc Ajax提交用户的分类绑定数据到数据库
     * /aliexpress/aliexpress_account_bind_category/do_insert_or_update
     */
    public function actionDo_insert_or_update(){
        $nowTime = date("Y-m-d H:i:s",time());
        $return = array('status' => 'success','msg' => '');
        $userID = Yii::App()->request->getParam('bindUserID');
        $firstCategory = Yii::App()->request->getParam('bindFirstCategory');
        $secondCategory = Yii::App()->request->getParam('bindSecondCategory');
        $thirdCategory = Yii::App()->request->getParam('bindThirdCategory');
        $modifyUserID = Yii::App()->user->id;
        $insertArray = array();        
        $insertArray['first_category'] = "$firstCategory";
        $insertArray['second_category'] = "$secondCategory";
        $insertArray['third_category'] = "$thirdCategory";
        $insertArray['modify_user'] = "$modifyUserID";
        $hasInsertStatus = AliexpressAccountBindCategory::model()->checkHasInsertStatus($userID);
        if ($hasInsertStatus){
            $insertArray['modify_time'] = $nowTime;
            if (AliexpressAccountBindCategory::model()->updateUserBindCategory($userID, $insertArray)){
                $return['msg'] = "绑定分类更新成功！";
            } else {
            ...
            $insertArray['account_id'] = "$userID";
            if (AliexpressAccountBindCategory::model()->insertUserBindCategory($insertArray)){
                $return['msg'] = "绑定分类成功！";
            } else {
                $return['status'] = "failure";
                $return['msg'] = "绑定分类失败！";
    ...
    /**
     * @desc 获取用户绑定的分类全部列表
     * /aliexpress/aliexpress_account_bind_category/get_all_bind_category
     */
    public function actionGet_all_bind_category(){
        $accountID = Yii::app()->request->getParam('accountID');
        $first = "\n\n<b>一级分类</b>\n\n". AliexpressAccountBindCategory::model()->getBindCategoryList($accountID,1);
        $second = "\n\n<b>二级分类</b>\n\n". AliexpressAccountBindCategory::model()->getBindCategoryList($accountID,2);
        $third = "\n\n<b>三级分类</b>\n\n". AliexpressAccountBindCategory::model()->getBindCategoryList($accountID,3);
        echo "<pre>" . $first . $second . $third . "</pre>";
    }
    ...
    public function actionAccount_list(){
        $model = UebModel::model('AliexpressAccountListForBindCategory');       
        $this->render('account_list', array('model'=>$model));
    }
