<?php
class LazadaZeroStockSku extends LazadaModel{
    ...
    public function tableName() {
        return 'ueb_lazada_zero_stock_sku';
    }

    /**
     * @desc 根据提供的sku数组查询表内是否存在数据
     * @param array
     * @return string 返回存在的sku
     */
    public function getExistZeroStockSku($list) {
        //var_dump($list['sku']);exit;
        $result = $this->dbConnection->createCommand()
                ->select('sku')
                ->from(self::tableName())
                ->where('sku =:sku',                    array(':sku'=>$list['sku']) )
                ->andWhere('seller_sku =:seller_sku',   array(':seller_sku'=>$list['seller_sku']) )
                ->andWhere('site_id =:site_id',         array(':site_id'=>$list['site_id']) )
                ->andWhere('account_id =:account_id',   array(':account_id'=>$list['account_id']) )
                ->andWhere('type =:type',               array(':type'=>$list['type']) )
                ->andWhere('status = 0')
                ->queryColumn();
        return $result;
    }
    ...
    public function getCount($condition) {
        $result = self::model()->count($condition);
        return $result;
    }
    ...
    public function getList($data) {
        ...
        if(isset($data['offset']) && !empty($data['offset'])){
            $command->offset = $data['offset'];
        }
        if(isset($data['order']) && !empty($data['order'])){
            $command->order = $data['order'];
        }
        if(isset($data['query_type']) && $data['query_type'] == 'queryColumn'){
    ...
    /**
     * @desc 获取是否恢复选项列表
     * @param string $isrestore
     * @return Ambigous <string>|multitype:string
     */
    public function getIsRestoreOptions($isrestore = null){
    	$options = array(
    		0=>'未恢复',
    		1=>'已恢复'
    	);
    	if($isrestore) return $options[$isrestore];
    	return $options;
    }
    ...
    public function addtions($datas){
    	...
    	foreach ($datas as &$data){
            ...
            //账号名称
            $data['site_id'] = LazadaSite::getSiteList($data['site_id']);
    		//状态
    		$data['status'] = $this->getStatusOptions($data['status']);
    		//类型
    		$data['type'] = $this->getTypeOptions($data['type']);
    		//是否恢复
    		$data['is_restore'] = $data['is_restore']?'已恢复':'未恢复';
    	}
    	return $datas;
    }
    ...
    public function getAccountList(){
    	if(self::$accountPairs == null)
    		self::$accountPairs = self::model('LazadaAccount')->getAccountList();
    ...
    public function updateData($params,$where){
        if(empty($params)) return false;
        return $this->getDbConnection()->createCommand()->update($this->tableName(), $params, $where);
    }
}