 ...
 * @since 2015-08-07
 */ 
class LazadaLog extends LazadaModel{
    ...
    /** @var 同步订单 */
    const EVENT_SYNCORDER      = 'syncorder';

    /** @var 更新COD订单交易状态 */
    const EVENT_UPDATE_TRANS_STATUS      = 'update_trans_status';    

    const EVENT_PRODUCT_SELLER_RELATION = 'lazada_product_seller_relation';

    /** @var 清除三个月前日志 */
    const EVENT_CLEAR_LOG = 'clear_log';

    /** @var 自动上传图片 */
    const AUTO_UPLOAD_IMAGES = 'autouploadimages';

    /** @var 自动修改产品价格 */
    const CHANGE_PRICE_RECORD = 'change_price_record';
    ...
    public function getAccountList(){
            if(self::$accountPairs == null)
                    self::$accountPairs = self::model('LazadaAccount')->getShortAccountNameList();
    ...
    public function getEventOptions($event = null){
            ...
            $eventOptions = array(
                ...
                self::EVENT_PRODUCT_SELLER_RELATION => '产品销售关系',
                self::EVENT_GETORDER                => '下载订单',
                self::EVENT_ORDERITEMS              => '下载订单明细',
