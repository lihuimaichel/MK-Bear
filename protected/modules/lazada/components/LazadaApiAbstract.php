 ...
 * @since 2015-08-07
 */
abstract class LazadaApiAbstract implements PlatformApiInterface {
    ...
    public function setAccount($accountID,$siteID=null){
        ...
        //获取账号相关信息
        $accountInfo = LazadaAccount::model()->getApiAccountByIDAndSite($this->accountID, $siteID);
//        $lazadaKeys = ConfigFactory::getConfig('lazadaKeys');
        $this->accountID        = $accountID;
        $this->_userID          = $accountInfo['email'];
        $this->_token           = $accountInfo['token'];
        $this->_url             = $accountInfo['server_url'];
        return $this;
    }
...
//        $lazadaKeys = ConfigFactory::getConfig('lazadaKeys');
    ...
    public function sendRequest() {
        ...
        try {
            ...
            if($this->_httpMethod=='GET'){
                $url = $this->getUrl();
                //MHelper::writefilelog($path.'Request_'.date("H").'.log', date("Y-m-d H:i:s").' #####request##### '.$url."\r\n\r\n");//test
                ...
                //MHelper::writefilelog($path.'Request_'.date("H").'.log', date("Y-m-d H:i:s").' #####request##### '.$this->_requestbody."\r\n\r\n");//test
                $response = Yii::app()->curl
                            ->setOption(CURLOPT_TIMEOUT,300)->setOption(CURLOPT_CONNECTTIMEOUT,300)
                            ->post($this->getUrl(), $this->_requestbody);
            }
            //MHelper::writefilelog($path.'Response_'.date("H").'.log', date("Y-m-d H:i:s").' ######response#### '.$response."\r\n\r\n");//test
    ...
    public function getIfSuccess(){
    	if( isset($this->response->Head->RequestId)) {
    		return true;
    	}else{
    		return false;
    	}
