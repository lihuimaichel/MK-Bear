 ...
 *
 */
class LazadaproductnewController extends UebController {
	...
	public function accessRules() {
		return array(
			array(
				'allow',
				'users' => ('*'),
				'actions' => array('getproducts', 'checkgetproducts'),
			),
		);
	}
	 ...
	 * @author yangsh
	 * @since  2016-10-07
	 * @link  /lazada/lazadaproductnew/getproducts
	 *        /lazada/lazadaproductnew/getproducts/account_id/9
	 */
	public function actionGetproducts() {
        ...
        $stateArr = array(
        	GetProductsRequestNew::PRODUCT_STATUS_ALL,//'all',  包括了live inactive, 除deleted
        	GetProductsRequestNew::PRODUCT_STATUS_DELETED,//'deleted',
        );        
	 ...
	 * @author yangsh
	 * @since  2016-10-07
	 * @link  /lazada/lazadaproductnew/checkgetproducts
	 *        /lazada/lazadaproductnew/checkgetproducts/account_id/1
	 */
	public function actionCheckgetproducts() {
		...
		$accountID 		= trim(Yii::app()->request->getParam('account_id',''));
		$state 			= trim(Yii::app()->request->getParam('state',''));//产品状态
		$type 			= trim(Yii::app()->request->getParam('type','1'));//默认1:更新时间,2:创建时间
	        ...
	        if (!empty($res)) {
	            foreach ($res as $row) {
	                ...
	                if (empty($skuOnline)) {
                        ...
                        $request->setSellerSku($skuOnline[0]['seller_sku']);
                        $request->setStatus('inactive');
                        $request->setSiteID($accountInfos[$accountID]['site_id']);
                        $response = $request->setAccount($accountInfos[$accountID]['account_id'])->setRequest()->sendRequest()->getResponse();
            ...
            try{
                ...
                do{
                    $command = $lazadaProductModel->getDbConnection()->createCommand()
                        ...
                        ->select("id,sku,account_auto_id,account_id,quantity,seller_sku,site_id")
                    ...
                    if($sellSkuArr){
	                    $request = new ProductUpdateRequest();
	                    $request->setSellerSku($sellSkuArr);
	                    $request->setStatus('inactive');
	                    $request->setSiteID($site_id);
	                    $response = $request->setAccount($account_id)->setRequest()->sendRequest()->getResponse();
	                    if ($request->getIfSuccess()) {
	                        $this->_model->batchUpdateStatus($idArr, LazadaProduct::PRODUCT_STATUS_INACTIVE);
	                        foreach ($addData as $logData) {
	                        	$lazadaLogOfflineModel->savePrepareLog($logData);
	                        }        
	                    }
	                }
