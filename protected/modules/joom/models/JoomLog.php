 ...
 * @since 2015-06-22
 */ 
class JoomLog extends JoomModel{
    ...
    const EVENT_UPDATE_PRODUCT_PRICE = 'update_price';

    const EVENT_UPDATE_PRODUCT_STOCK= 'update_stock';
    ...
    /** @var 手动拉取订单 */
    const EVENT_MANUAL_GETORDER = 'get_manual_order';    
    ...
    public function saveEventStatus($eventName, $logID, $status,$message = ''){
        $tableName = self::tableName().'_'.$eventName;
        if( $eventName == JoomShipment::EVENT_ADVANCE_SHIPPED || $eventName == JoomShipment::EVENT_UPLOAD_TRACK ){
        	$updateData =  array(
        			'status'        => $status,
        			'end_time'      => date('Y-m-d H:i:s'),
        			'message'		=> $message,
