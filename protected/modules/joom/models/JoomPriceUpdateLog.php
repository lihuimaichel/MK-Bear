...
class JoomPriceUpdateLog extends JoomModel {
	...
	public function tableName() {
		return 'ueb_joom_price_update_log';
	}
	...
	public function reUploadUpdateInfo($uploadLog)
    {
        ...
        try {
            ...
            JoomVariants::model()->updateVariantDataOnline($uploadLog['online_sku'], $uploadLog['account_id'], array('price' => $uploadLog['new_listing_price']));
    ...
    public function updateExpiredInfo($listingId, $accountId, $timeInterval = '-3 day')
    {
        ...
        $updateData = array(
            ...
            'message'=> Yii::t('joom', 'Has latest info within 3 days')
        );
    ...
    public function getUpdateTypeText($action)
    {
        ...
        switch ($action){
            case self::UPDATE_TYPE_PRICE_UP:
                $actionText = Yii::t('joom', 'Update price up');
                break;
            case self::UPDATE_TYPE_PRICE_DOWN:
                $actionText = Yii::t('joom', 'Update price down');
                break;
            case self::UPDATE_TYPE_WEIGHT_UP:
                $actionText = Yii::t('joom', 'Update price weight up');
                break;
            case self::UPDATE_TYPE_WEIGHT_DOWN:
                $actionText = Yii::t('joom', 'Update price weight down');
                break;
        }
        return $actionText;
    }
    ...
    public function getUpdateTypeFullText($info)
    {
        ...
        switch ($info->update_type) {
            case self::UPDATE_TYPE_PRICE_UP:
            case self::UPDATE_TYPE_PRICE_DOWN:
                $fullText .= Yii::t('joom', 'Old Product Cost:');
                $fullText .= $info->old_value;
                $fullText .='<br>';
                $fullText .= Yii::t('joom', 'New Product Cost:');
                $fullText .= $info->new_value;

                break;
            case self::UPDATE_TYPE_WEIGHT_UP:
            case self::UPDATE_TYPE_WEIGHT_DOWN:
                $fullText .= Yii::t('joom', 'Old Product Weight:');
                $fullText .= $info->old_value;
                $fullText .='<br>';
                $fullText .= Yii::t('joom', 'New Product Weight:');
                $fullText .= $info->new_value;
                break;
        }
    ...
    public function getActionText($value)
    {
        ...
        if ($dateInterval->d < 2 && $value->upload_status != self::LOG_STATUS_SUCCESS &&   $value->upload_times < 2 ) {
            return '<a target="ajaxTodo" href="'.Yii::app()->createUrl('joom/joompriceupdatelog/reuploadpriceupdate',
                    array
                ('id'=>
                    $value->id)).'">'
                .Yii::t('joom', 'Reload')
    ...
    public function attributeLabels() {
        return array(
            ...
            'last_user_id'=> '最后操作人员',
            'message'							=>	'提示',
            'old_listing_price'					=>	Yii::t('joom', 'Old Listing Price'),
            'old_profit_rate'					=>	Yii::t('joom', 'Old Listing Profit'),
            'new_listing_price'					=>	Yii::t('joom', 'New Listing Price'),
            'new_profit_rate'					=>	Yii::t('joom', 'New Listing Profit'),
        );
    }
    /**
    ...
    public function filterOptions() {
        ...
        $result = array(
            ...
            array(
                ...
                'type'=>'dropDownList',
                'search'=>'=',
                'data'=> JoomAccount::model()->getIdNamePairs()
            ),
