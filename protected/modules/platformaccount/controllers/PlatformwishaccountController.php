<?php
class PlatformwishaccountController extends UebController {
	...
	public function init() {
		$this->_model = new PlatformWishAccount();
		parent::init();
	}
	...
	public function actionAdd(){
		...
		//获取所属部门
		$departmentList = Department::model()->getDepartmentByKeywords('wish');
	...
	public function actionAddsave(){
		...
		if($result){
			// $data=$this->_model->wishAccountAuthorize($result, $redirectUri);
			// $url=$data->_get_wish_code_url;
			echo $this->successJson(array('message' => '添加成功', 'url' => '', 'aid' => $result));
		}else{
	...
	public function actionTokensave(){
		...
		$request = new WishAccessTokenRequest();
	...
	public function actionEdit(){
		...
		if($_POST){
			$accountParam = Yii::app()->request->getParam('PlatformWishAccount');
			...
			if($result){
				$jsonData = array(
					'message' => '更改成功',
					'forward' =>'/platformaccount/platformwishaccount/list',
					'navTabId'=> 'page' . PlatformWishAccount::getIndexNavTabId(),
	...
	public function actionUpdatetoken(){
		...
		foreach ($idsArr as $accountID) {
			...
			$request = new WishRefreshTokenRequest;
			$response = $request->setAccount($accountID)->setRequest()->sendRequest()->getResponse();
			if($request->getIfSuccess()){
				$data = array(
					'access_token'       => $response->data->access_token,
	...
	public function actionTooms(){
		...
		$wishIds = $wishAccountModel->getWishAccountID();
		...
		foreach ($idsArr as $accountID) {
			...
			if(in_array($accountID, $wishIds)){
			    ...
			    $result = $wishAccountModel->getDbConnection()->createCommand()->update($wishAccountModel->tableName(), $data, "id='{$accountID}'");
				if($result){
					$omsStatus = 1;
				}
			    ...
			    $data['id'] = $accountID;
			    $res = $wishAccountModel->getDbConnection()->createCommand()->insert($wishAccountModel->tableName(), $data);
				if($res){
					$omsStatus = 1;
				}
			}
	...
	public function actionReauthorization(){
		...
		if($_POST){
			// $data=$this->_model->wishAccountAuthorize($info->id, $info->redirect_uri);
			// $url=$data->_get_wish_code_url;
			echo $this->successJson(array('message' => '添加成功', 'url' => ''));
			Yii::app()->end();
		}
	...
	/**
	 * 自动更新token程序
	 * @link /platformaccount/platformwishaccount/autorunaccesstoken/id/1/times/1490878847
	 */
	public function actionAutorunaccesstoken(){
        ...
        //创建运行日志		
		$logId = $logModel->prepareLog($accountID, PlatformAccountLog::EVENT_WISH_RUNING_TOKEN, Platform::CODE_KF);
		if(!$logId) {
			echo Yii::t('wish_listing', 'Log create failure');
			Yii::app()->end();
		}
		//检查账号是可以提交请求报告
		$checkRunning = $logModel->checkRunning($accountID, PlatformAccountLog::EVENT_WISH_RUNING_TOKEN, Platform::CODE_KF);
		if(!$checkRunning){
		...
		if($accountID && $invalidTime){
			try{
				...
				// $omsAccountModel = new OmsWishAccount();
				$wishAccountModel = new WishAccount();
				...
				$refreshTokenRequest = new WishRefreshTokenRequest();
				...
				if ($refreshTokenRequest->getIfSuccess()) {
					...
					try{
						$dbtransaction = PlatformWishAccount::model()->getDbConnection()->beginTransaction();
						$this->_model->updateByPk($accountID, $data);
						$wishAccountModel->getDbConnection()->createCommand()->update($wishAccountModel->tableName(), $omsData, "id='{$accountID}'");
						// $omsAccountModel->updateData($omsData, 'wish_id = :id', array(':id'=>$accountID));
	...
	public function actionSetapi(){
		...
		if($_POST){
			$data = Yii::app()->request->getParam('PlatformWishAccount');
