 ...
 * @since 2017-03-16
 */
class PaytmProductWarehouses extends PaytmModel {
    ...
    public function tableName() {
        return 'ueb_paytm_product_warehouses';
    }
     ...
     */
    public function updateProductWarehouses($warehousesArray){
        $nowTime = date('Y-m-d H:i:s');
        $return = array('status'=>'success','msg'=>'');
        $warehousesArray['update_time'] = $nowTime;
        if (isset($warehousesArray) && !empty($warehousesArray) && isset($warehousesArray['product_id'])){
            $pkId = $this->getDbConnection()->createCommand()
                                            ->from($this->tableName())
                                            ->select("id")
                                            ->where("product_id=:product_id_arr",array(":product_id_arr" => $warehousesArray['product_id']))
                                            ->queryScalar();
            if($pkId){
                $isOk = $this->getDbConnection()->createCommand()
                                                ->update($this->tableName(), $warehousesArray, "id=:id", array(':id'=>$pkId));
                if (!$isOk){
                    $return['status'] = 'failure';
                    $return['msg'] = '数据插入失败！';
                }
            } else {
                $warehousesArray['creat_time'] = $nowTime;
                $isOk = $this->getDbConnection()->createCommand()
                                                ->insert($this->tableName(), $warehousesArray);
                if($isOk) {
