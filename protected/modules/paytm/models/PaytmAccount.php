 ...
 * @since 2017-02-28
 */
class PaytmAccount extends PaytmModel{
    ...
    public static function getAbleAccountList(){
        return PaytmAccount::model()->dbConnection->createCommand()
                       ->select('*')
                       ->from(self::tableName())
    ...
    public static function getAccountInfoById($id){
        return PaytmAccount::model()->dbConnection->createCommand()->select('*')->from(self::tableName())->where('id = '.$id)->queryRow();
    }
    ...
    public static function getByAccountName($accountName) {
    	return PaytmAccount::model()->getDbConnection()->createCommand()
		    	->select('*')
		    	->from(self::tableName())
    ...
    public static function getAbleAccountListByFilterId($ids){
    	if(is_array($ids) && $ids)
    		$ids = implode(",", $ids);
    	$command = PaytmAccount::model()->dbConnection->createCommand()
					    	->select('*')
					    	->from(self::tableName())
					    	->order("account_name asc")
    ...
    public static function getAccountInfoByIds($id){
    	...
    	$sql = PaytmAccount::model()->dbConnection->createCommand()->select('*')->from(self::model()->tableName())->where('id IN ('.implode(',', $id).')');
    	if( $flag ){
    		return $sql->queryRow();
    	}else{
    		return $sql->queryAll();
	...
	public static function getIndexNavTabId() {
		return Menu::model()->getIdByUrl('/Wish/PaytmAccount/list');
	}
	/**
