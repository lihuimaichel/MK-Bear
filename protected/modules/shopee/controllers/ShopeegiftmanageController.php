 ...
 * shopee赠品管理
 */
class ShopeegiftmanageController extends UebController {
	
    protected $_model = null; 
    
    public function  init(){
        $this->_model = new ShopeeGiftManage();
        parent::init();
    }
    ...
    public function actionAdd(){
        if(Yii::app()->request->isPostRequest && 'add' == Yii::app()->request->getParam('act') ){
        	$formData = Yii::app()->request->getParam('ShopeeGiftManage');
            ...
            if($this->_model->insertData($data)){
                $jsonData = array(
					'message'      => '添加成功',
					'forward'      => '/shopee/shopeegiftmanage/list',
					'navTabId'     => 'page'.Menu::model()->getIdByUrl('/shopee/shopeegiftmanage/list'),
        ...
        $accountList = array(0=>'请选择')+ShopeeGiftManage::getAccountList();
    ...
    public function actionEdit() {
        ...
        if(Yii::app()->request->isPostRequest 
            ...
            $isExits = $this->_model->getOneByCondition('id',"account_id='{$accountId}' and sku='{$sku}' and gift_sku='{$giftSku}' ");
            if(!empty($isExits)){
                echo $this->failureJson(array( 'message' => '赠品配置已存在'));
                exit;
            }
