	    ...
	    <div class="dot7" style="padding:5px;">
	       ...
	       <?php
            $form = $this->beginWidget('ActiveForm', array(
                'id' => 'amazon_warehouse_config_add',
                ...
                'action' => Yii::app()->createUrl('amazon/amazonwarehouseconfig/savedata'), 
                'htmlOptions' => array(        
                    'class' => 'pageForm',         
                )
            ));
            ?> 
            	<div class="formBar">
                    <ul> 
                        ...
                        <a href="<?php echo Yii::app()->createUrl('amazon/amazonwarehouseconfig/index');?>" target="navTab" style="display:none;" class="display_list"><?php echo Yii::t('system', 'Save');?></a>
                    </ul>
                </div>
        		<table class="dataintable_inquire productAddInfo" width="100%" cellspacing="1" cellpadding="1" border="0">
				    <tbody>				    	
				        <?php if($warehouseList):?>				        
                        <tr>
				            <td width="15%" style="font-weight:bold;">仓库列表</td>
				            <td width="85%" height="420">
					            <div style="margin:30px 80px 0 10px;">
					            	<?php foreach ($warehouseList as $warehouse):?>
					                <div style="float:right; display:inline; padding:5px;width:150px;border:1px dashed #e1e1e1;">
	                             		<?php echo CHtml::label($warehouse['name'], $warehouse['id'], array('style'=>'float:left')); ?>
...
function saveInfo(){
	$.ajax({
			type: 'post',
			url: $('form#amazon_warehouse_config_add').attr('action'),
			data:$('form#amazon_warehouse_config_add').serialize(),
