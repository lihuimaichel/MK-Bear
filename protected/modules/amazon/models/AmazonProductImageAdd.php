 ...
 * @author Gordon
 */
class AmazonProductImageAdd extends AmazonModel
{
    ...
    public static function getImagesPathFromRestfulBySku($sku)
    {
//        $host = 'http://172.16.2.63:8084';
//        $path = 'image';

        $host = self::getRestfulAddress('image');
    ...
    public static function getImagesPathFromRestfulBySkuAndType($sku, $typeAlisa = null)
    {
        $imageList = array();
        $imagePathList = self::getImagesPathFromRestfulBySku($sku);
        if (!$imagePathList) {
            return $imageList;
        }
    ...
    public static function getImageUrlFromRestfulByFileName($filename, $sku, $type = 'normal', $width = 100, $height = 100)
    {
    ...
    public static function getImageUrlFromRestfulBySku($sku, $typeAlisa = null, $type = 'normal', $width = 100, $height = 100)
    {
        $imageUrlList = array();
        $imagePathList = self::getImagesPathFromRestfulBySku($sku);
        if (!$imagePathList) {
        ...
        foreach ($imagePathList as $info) {
            $imageUrlList[$info['main'] ? self::IMAGE_ZT_ALIAS : self::IMAGE_FT_ALIAS][$info['filename']] = self::getImageUrlFromRestfulByFileName($info['filename'], $sku, $type, $width, $height);
        }
     ...
     */
    public static function getRestfulAddress($path = null)
    {
        $config = ConfigFactory::getConfig('imageKeys');
        // if( !isset($config[ 'COMMON' ]) ){
        //     throw new CException(Yii::t('system', 'Server Does Not Exists'));
