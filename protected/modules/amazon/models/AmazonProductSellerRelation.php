 ...
 * @since 2016-03-28
 */
class AmazonProductSellerRelation extends AmazonModel{
	public $site_name;
	public $account_name;
    ...
    public function tableName() {
        return 'ueb_amazon_product_seller_relation';
    }
     ...
     * @author yangsh
     */
    public function getListByCondition($fields='*', $conditions, $params=array(), $order='', $group = '') {
        $cmd = $this->dbConnection->createCommand();
        $cmd->select($fields)
            ->from(self::tableName());
        if (!empty($params)) {
    ...
    public function batchChangeSellerToOtherSeller($oldAccountId, $oldSellerId, $newSellerId){
        $dbtransaction = AmazonProductSellerRelationLog::model()->getDbConnection()->beginTransaction();
        try {
    ...
    public function batchSetAccountListingToSeller($accountID, $sellerID){
        ...
        do{
            $productList = $this->getDbConnection()->createCommand()
                                    ->from(AmazonList::model()->tableName()." as p")
                                    ->leftJoin($this->tableName() . " as s", "s.account_id=p.account_id and s.online_sku=p.seller_sku and s.item_id=p.asin1")
                                    ->select("p.asin1 as item_id, p.sku, p.seller_sku as sku_online, p.account_id, s.seller_id")
                                    ->where("p.account_id='{$accountID}' and ISNULL(s.seller_id) and p.seller_status=1")
                                    ->limit($limit)
                                    ->queryAll();
            if(!empty($productList)){
                foreach ($productList as &$val){
                    ...
                    $params = array(':item_id'=>$val['item_id'], ':account_id'=>$val['account_id'], ':sku'=>$val['sku'], ':online_sku'=>$val['sku_online']);
    ...
    public function batchSetSkuListingToSeller($ids, $sellerID){
        ...
        $productList = $this->getDbConnection()->createCommand()
                            ->from(AmazonList::model()->tableName()." as p")
                            ->leftJoin($this->tableName() . " as s", "s.account_id=p.account_id and s.online_sku=p.seller_sku and s.item_id=p.asin1")
                            ->select("p.asin1 as item_id, p.sku, p.seller_sku as online_sku, p.account_id, s.seller_id")
                            ->where("ISNULL(s.seller_id) and p.seller_status=1")
    ...
    public static function getIndexNavTabId() {
        return Menu::model()->getIdByUrl('/amazon/amazonproductsellerrelation/list');
    }
    ...
    public static function getUnbindsellerNavTabId() {
        return Menu::model()->getIdByUrl('/amazon/amazonproductsellerrelation/unbindseller');
    }
    ...
    public function getBindSellerListByCondition($conditions, $params){
        return $this->getDbConnection()->createCommand()
                        ->select('id, item_id, sku, online_sku, account_id, seller_id')
                        ->from($this->tableName())
    ...
    public function getUnBindSellerListByCondition($conditions, $params){
        return $this->getDbConnection()->createCommand()
                    ->select('p.asin1 as item_id, p.sku, p.seller_sku as online_sku, p.account_id')
                    ->from(AmazonList::model()->tableName(). ' as p')
                    ->leftJoin($this->tableName(). ' as s', 's.account_id=p.account_id and s.online_sku=p.seller_sku and s.item_id=p.asin1')
                    ->where($conditions, $params)
    ...
    public function filterOptions(){
    	return array(
    			...
    			array(
    					...
    					'type'=>'dropDownList',
    					'search'=>'=',
    					'data'	=>	AmazonAccount::model()->getIdNamePairs(),
    					'htmlOption'=>array(
    							'size'=>'22'
    					)
    			),
