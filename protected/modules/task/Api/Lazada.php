...
class Lazada extends Api
{
	...
	private function save($task, $fetchArray, $params)
	{
		...
		$model = ('listing' == $task) ? LazadaWaitListing::model() : LazadaWaitOptimization::model();
						...
						if (empty($result)) {
							...
							switch ($task) {
								case 'listing':
									...
									break;
								case 'optimization':
									$listingStatus = LazadaProduct::getProductStatusByStatusText($v['listingStatus']);
									$updateArr = array_merge($updateArr, array('listing_status' => $listingStatus));
									break;
								default:
									$updateArr = array_merge($updateArr, array('sku_status' => $v['skuStatus']));
									break;
			...
			if (!empty($fetchArray)) {
				foreach ($fetchArray as $k => $v) {
					try {
						...
						$newData = array();
						foreach ($fieldMapping as $field => $value) {
						    if ('listing_status' == $field) {
                                $listingStatus = LazadaProduct::getProductStatusByStatusText($v['listingStatus']);
                                $newData[$field] = $listingStatus;
                            } else {
                                $newData[$field] = $this->processFieldMapping($value, $v);
                            }
						}
